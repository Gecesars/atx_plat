...
def create_app():
   ...
   # gmaps = googlemaps.Client(key=os.environ.get('GOOGLE_MAPS_API_KEY'))
    ...
    def gerardiagramas():
        ...
        # salva no banco
        resp = salvar_diagrama()
        if getattr(resp, "status_code", 200) != 200:
            return resp

        # lê para parse
        file_content = file.stream.read().decode('latin1', errors='ignore')
        horizontal_data, vertical_data, meta = parse_pat(file_content)
    ...
    def gerar_img_perfil():
        ...
        fresnel_radius = np.sqrt((1 / (4 * np.pi * frequency.to_value(u.Hz))) * (x * (rx_position_km - x) / rx_position_km) * 1e12)
        ...
        ax.plot(x, fresnel_top,    'm--', label='Topo 1ª Zona Fresnel')
        ax.plot(x, fresnel_bottom, 'm:',  label='Fundo 1ª Zona Fresnel')
        ...
        ax.text(0.5, 0.5, annotation_text, transform=ax.transAxes, fontsize=11,
                va='center', ha='center', bbox=dict(boxstyle='round,pad=0.3', ec='black', fc='lightyellow'))
